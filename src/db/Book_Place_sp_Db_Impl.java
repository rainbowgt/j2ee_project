...
import db.BaseDb.*;
public class Book_Place_sp_Db_Impl implements Book_Place_sp_Db {
    ...
    public int order_insert(BookSpBean ob)
    {
        ...
        String insert="insert into book_place_sp(idplace,idroom,Book_time,vacancy,max)values(?,?,?,?,?)";
        try{
            db=new BaseDb();
    ...
    public ResultSet order_select()
    {
        BaseDb db=null;
        PreparedStatement pstm=null;
        String select="SELECT * FROM book_place_sp where vacancy>0";
        ResultSet rs=null;
        try{
            db=new BaseDb();
        ...
        {
            e.printStackTrace();
            System.out.println("查询Book_Place_sp_Db数据异常");
        }
        //  db.close();
        return rs;
    }
    ...
    public int order_delete(BookSpBean ob)
    {
        ...
        BaseDb db = null;
        PreparedStatement pstm = null;
        String delete="DELETE FROM book_place_sp where idplace=? AND idroom=? AND Book_time=?";
        try{
            db=new BaseDb();
    ...
    public int order_update(BookSpBean ob)
    {
        ...
        BaseDb db = null;
        PreparedStatement pstm = null;
        String update="UPDATE book_place_sp SET vacancy=? where idplace=? AND idroom=? AND Book_time=?";
        try{
            db=new BaseDb();
